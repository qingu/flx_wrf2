...
#   die "Resolved IO for called subs, now use it!\n" if $f =~ /advance/;
    	...
    	print "determine_argument_io_direction_rec(): $f is LEAF\n";
...
#    my $called_sub      = "";
    ...
    my $called_sub_args = analyse_src_for_iodirs($stref,$f);
    print Dumper( $called_sub_args  );
##    if ( defined $srcref ) {
...
##    }
    ...
    die Dumper($Sf->{'RefactoredArgs'}) if $f=~/caldate/;
...
#            print Dumper( $tags );
            ...
            if ( exists $tags->{'SubroutineCall'} ) {
                ...
                my $Sname = $stref->{'Subroutines'}{$name};
                ( my $iodirs, $stref ) =
                  get_iodirs_from_subcall( $stref, $f, $index ); die "FIXME: surely we should use the recursion for this!?";
                for my $var ( keys %{$iodirs} ) {
                    if ( exists $args{$var} ) {
                        if ( $iodirs->{$var} eq 'In' ) {
                            if ( $args{$var} eq 'Unknown' ) {
