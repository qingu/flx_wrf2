...
# Executive decision: do this only for the routines to be translated to C/OpenCL
			for my $called_sub (
				keys %{ $stref->{'Subroutines'}{$f}{'CalledSubs'} } )
			{
...
sub create_new_include_statements {
	...
	for my $inc ( keys %{ $stref->{'Subroutines'}{$f}{'Globals'} } ) {
		print "INC: $inc, root: $stref->{'IncludeFiles'}{$inc}{'Root'} \n"
		  if $V;
		if ( exists $stref->{'Subroutines'}{$f}{'CommonIncludes'}{$inc}
...
sub resolve_globals {
	...
	# We only come here when the recursion is done.
...
/(logical|integer|real|double\s*precision|character|character\*?(?:\d+|\(\*\)))\s+(.+)\s*$/
				...
				@globs=( @globs, look_for_variables( $stref, $f, $line, $tvars ) );				
			}    # for each line			
			if ($V) {
...
# We do a recusive descent for all called subroutines, and for the leaves we do the analysis
	sub determine_argument_io_direction_rec {
		...
		if ( exists $stref->{'Subroutines'}{$f}{'CalledSubs'} ) {
			...
			{
				$stref =
				  determine_argument_io_direction_rec( $calledsub, $stref );
			}
		}
...
"INFO: ARG $call_arg in call to $name in $f is an expression\n"
						...
						for my $maybe_arg (@maybe_args) {
							if ( exists $args{$maybe_arg}
								and not
								exists $called_arg_iodirs->{$maybe_arg} )
							{
								print
"INFO: Setting IO dir for $maybe_arg in call to $name in $f to In\n"
								...
								if (
									...
									{$f} == 1
									and $stref->{'Subroutines'}{$name}
									{'RefactoredArgs'}{$sig_arg}{'IODir'} ne
									'In'
								  )
								{
									print
"INFO: $name in $f is called only once; $sig_arg is an expression, setting IODir to 'In'\n"
						...
						if (
							scalar
							keys %{ $stref->{'Subroutines'}{$name}{'Callers'} }
							== 1
							and $stref->{'Subroutines'}{$name}{'Callers'}{$f} ==
							1
...
"WARNING: IO direction for $var in call to $name in $f is Unknown\n"
		...
		for my $arg ( keys %args ) {
			...
			$stref->{'Subroutines'}{$f}{'RefactoredArgs'}{$arg}{'Shape'} =
			  $shape;
		}
		$stref =
				  ...
				  )
				{
					...
					for my $var (@tvars) {
						...
						if ( exists $stref->{'Functions'}{$tvar} ) {
							...
							if (
								not
								exists $stref->{'Functions'}{$tvar}{'Lines'} )
							{
	...
	sub get_commons_params_from_includes {
		...
		if ( defined $srcref ) {
			...
			if ($V) {
				print "\nCOMMONS for $f:\n\n";
				for my $v (
					sort keys %{ $stref->{'IncludeFiles'}{$f}{'Commons'} } )
				{
					print $v, "\n";
				}
			}
	 ...
	 #	               	( $prevline, my $comment ) = split( /\s+\!/, $prevline );
			   ...
			   # and substr($prevline,-length($line),length($prevline)) ne $line
				...
				if (    $f =~ /^include/
					...
					and substr( $prevline, -length($line), length($prevline) )
					eq $line )
				{
 ...
 # -----------------------------------------------------------------------------
	sub reshape_args {
		...
		for my $refarg ( @{$refargs} ) {
			...
			if ( $k == 1 ) {
				my $shape =
				  $stref->{'Subroutines'}{$f}{'RefactoredArgs'}{$refarg}
				  {'Shape'};
				if ( defined $shape ) {
					...
					die keys %{ $stref->{'Subroutines'}{$f}{'RefactoredArgs'}
						  {$refarg} };
				}
				my $type =
				  $stref->{'Subroutines'}{$f}{'RefactoredArgs'}{$refarg}
